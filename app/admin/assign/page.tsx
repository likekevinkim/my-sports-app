...
function AssignContent() {
  ...
  const fetchData = async () => {
    ...
    if (data && data.length > 0) {
      // DB에서 가져올 때 각 선수의 실력 정보를 포함하도록 처리 (있을 경우)
      const formatted: ScheduleSlot[] = data.map((d: any) => ({
        ...
        players: d.player_names ? d.player_names.split(', ').map((name: string, i: number) => ({
          ...
          name: name,
          // 여기서 실제 프로필의 실력 레벨을 매칭하고 싶다면 별도의 조인이 필요하나, 
          // 현재는 스케줄 테이블에 저장된 player_ids를 기반으로 배정 로직에서 처리됨
        })) : []
      }));
  ...
  const runAutoAssign = async () => {
    ...
    timeSlots.forEach((time, idx) => {
      ...
      tempSchedule[idx].players = selected.map(p => ({ 
        id: String(p.id), 
        name: p.name,
        skill_level: p.skill_level || 0 // 실력 데이터 포함
      }));
    });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const insertData = schedule.map((slot) => ({
        ...
        avg_skill: slot.players.length > 0 
          ? parseFloat((slot.players.reduce((sum, p) => sum + (p.skill_level || 0), 0) / slot.players.length).toFixed(1))
          : 0
      }));
      ...
      <div className="w-full max-w-[430px] bg-white min-h-screen shadow-2xl flex flex-col p-6 overflow-y-auto">
        ...
        {isEditable && (
          <button 
            ...
            disabled={loading}
            className="w-full bg-blue-600 text-white py-4 rounded-[24px] font-black text-sm mb-8 shadow-xl active:scale-95 transition-all"
          >
                ...
                <div className="flex justify-between items-center mb-4 px-1">
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      {isShortage && (
                        <span className="bg-red-500 text-white text-[9px] px-2 py-0.5 rounded-full font-bold animate-pulse uppercase">
                          Shortage
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-0.5 flex items-center gap-1.5">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Avg Skill</span>
                ...
                <div className="space-y-2">
                  {slot.players.map((p) => (
                    ...
                    >
                      ...
                      {/* 개별 실력 점수 살짝 표시 (선택 사항) */}
                      <span className="text-[10px] font-bold text-slate-300">Lv.{p.skill_level || 0}</span>
                    </div>
                  ))}
                  {slot.players.length === 0 && <div className="py-8 border-2 border-dashed border-slate-200 rounded-2xl text-center text-[10px] text-slate-300 font-bold italic">DROP PLAYER HERE</div>}
                </div>
              </div>
            );
          })}
        ...
        {isEditable && (
          <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] p-6 bg-gradient-to-t from-white via-white to-transparent">
            <button onClick={handleConfirm} disabled={saving} className="w-full bg-slate-900 text-white py-5 rounded-[30px] font-black text-xl shadow-2xl active:scale-95 transition-all">
              {saving ? 'SAVING...' : '최종 스쿼드 확정 게시'}
            </button>
          </div>
        )}
      </div>
...
export default function AssignPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen font-black text-slate-300 animate-pulse">LOADING...</div>}>
      <AssignContent />
    </Suspense>
  );
}